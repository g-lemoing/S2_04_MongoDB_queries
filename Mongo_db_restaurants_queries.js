...
db.restaurants.find({$and: [{cuisine: {$not: {$eq: 'American '}}, 'grades.score': {$gt: 70}, 'address.coord.1': {$lt: -65.754168}}]});
...
db.restaurants.find({cuisine: {$not: {$eq: 'American '}}, 'grades.score': {$gt: 70}, 'address.coord.1': {$gt: -65.754168}});
...
db.restaurants.find({cuisine: {$not: {$eq: 'American '}}, 'grades.grade': 'A', borough: {$ne: 'Brooklyn'}}).sort({cuisine: -1});
...
db.restaurants.find({$and: [{cuisine: 'Seafood'}, {$nor:[{cuisine: {$in:['Chinese', 'American ']}}, {name: {$regex:"^Wil"} }]}]},{restaurant_id:1, name:1, borough:1, cuisine:1, _id:0});
...
db.restaurants.find({grades: {$elemMatch: {grade: "A", score: 11, date: ISODate("2014-08-11")}}}, {restaurant_id: 1, name: 1, borough: 1, cuisine: 1, _id: 0});
...
db.restaurants.find({'grades.1.grade': 'A', 'grades.1.score': 9, 'grades.1.date': ISODate('2014-08-11')}, {restaurant_id:1, name:1, grades:1, _id:0});
...
db.restaurants.find({"address.coord.1": {$lte:52, $gte:42}}, {restaurant_id:1, name:1, address: 1, _id:0 });
...
db.restaurants.find({"address.coord": { $type: 1 }}, {restaurant_id: 1, name: 1, address: 1, _id: 0});
